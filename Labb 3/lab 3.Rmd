...
for (i in 2:n_samp){
  ...
  for (j in 1:nrow(X)){
    ...
    wi[j,i-1] <- rpg(1,h=1,X[j,]%*%beta_samples[i-1,])
  }
  omega <- diag(wi[,i-1])
...
$$IF = 1 + 2 \cdot \sum_{k=1} ^\inf\rho_k  $$

where $\rho_k = Corr(\thetaî,\theta^{(i+k)})$
...
There is autocorrelation within the draws but we still get a relatively low value after 3000 draws. 
