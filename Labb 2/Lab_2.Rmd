...
for (i in 1:1000){
  ...
  # beta posterior mean
  mu_n <- solve(xTx + omega0) %*% (xTx %*% beta[i,] + omega0 %*% t(mu0)) 
  
  # df, sigma posterior
  vSigma_n <- v0 * sigma0 + (yTy + mu0 %*% omega0 %*% t(mu0)) - (t(mu_n) %*% omega_n %*% mu_n) 
  
  # joint sigma posterior distribution
  sigmaPost[i] <- (vSigma_n) / rchisq(1, v_n) 
...
The individual posterior parameters follow a normal distribution, except the $\sigma^2$ which has a distribution skewed to the right.
...
The posterior probability intervals do contain a good amount of data for a large part of time, however not perfect. The model is limited in that it only is a quadratic model for data seemingly looking to be more complex, thus the posterior can only be optimized to a certain point. An equal tail interval means the same density mass is outside both boundaries.
