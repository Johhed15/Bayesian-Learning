...
plot(lin$temp, x=lin$time, type='l', xlim=c(0,1),ylim=c(-20,30))
for (i in 1:1000){
...
logisticpost <- function(betas,y,X,tau){
  ...
  B_prior <- rmvnorm(1,rep(0,7),diag(tau ^ 2,7))
  
  #linpred <- X%*%betas
  #loglik <- (exp(linpred)/(1+exp(linpred)))
...
#OptimRes <- optim(initVal,logPost,gr=NULL,y,X,tau,Sigma,method=c("BFGS"),control=list(fnscale=-1),hessian=TRUE)
