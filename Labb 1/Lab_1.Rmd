...
posterior <- function(k){
  ...
  exp(k * (sum(cos(y-mu)))-k)/ (besselI(k, 0)^n)
...
At element 217 the posterior density is the highest at 0.004603008, which we use to find the maximum (i.e. mode) value of the [0,10] sequence.

```{r}
seq(0,10,length.out=1000)[which.max(norm)]
```
This is the approximative posterior mode of $\kappa$. If the sequenced numbers would increase the approximate value would be closer to true convergence.
