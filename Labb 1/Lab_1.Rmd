...
The probability of the Gini coefficient being below or over this interval is 2.5%.

## d

```{r}
gini_dens <- density(gini) 

sort_dens <- sort(gini_dens$y,decreasing=TRUE)

cdf <- cumsum(sort_dens)/sum(sort_dens)

lower_index <- min(which(cdf >= 0.025))
upper_index <- max(which(cdf <= 0.975))

lower_value <- gini_dens$x[lower_index]
upper_value <- gini_dens$x[upper_index]

# HPDI Interval
HPDI_interval <- c(lower_value, upper_value)
