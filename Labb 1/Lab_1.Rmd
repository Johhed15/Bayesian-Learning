...
```{r}
plot(density(post_sigma),main="n*tau/rchisq density",xlim=c(0,1))
...
$$\text{This is a constant and can therfore be ignored} = \frac{1}{(2 \pi)^2}$$


$$\text{Likelihood = }\frac{ \exp (\kappa \cdot \sum^n_{i=1} \cos (y-\mu))}{(I_0(\kappa))^n}\cdot$$


$$\text{Posterior}= \ p(y \mid \mu, \kappa) \cdot p(K) = \frac{ \exp (\kappa \cdot \sum^n\cos (y-\mu))}{(I_0(\kappa))^n} \cdot p(K) $$

$$\text{Posterior}= \ p(y \mid \mu, \kappa) \cdot p(K) = \frac{ \exp (\kappa \cdot( \sum^n\cos (y-\mu))-0.5)}{(I_0(\kappa))^n} $$
...
vonMises <- function(mu, k, y){
  ...
  prod(exp(k * cos(y-mu)) / (2*pi*besselI(k, 0))) # det var y istället för k i besselI(därför vi fick Nans)
...
posterior <- function(k){
  ...
  exp(k * (sum(cos(y-mu)))-0.5)/ (besselI(k, 0)^n)
  
}

# applying a grid of k values for the posterior function
posterior_3 <- sapply(seq(0.01,10,length.out=1000),posterior)

norm <- posterior_3/sum(posterior_3)
